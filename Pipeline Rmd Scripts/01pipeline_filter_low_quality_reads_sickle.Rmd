# Filter low quality reads using trimmer from sickle (https://github.com/najoshi/sickle).
...
##Generate input and out filenames wpath, with dimension
R1.input<-NA
...
##File commands to be run by Sickle for quality filtering
...
```{r,echo=FALSE,eval=TRUE,include=FALSE,messages=TRUE,error=TRUE}
###Compare Unfiltered and Filtered Sequence Counts
...
filt_id1 <- substring(filt_dt$File, 78,80)
...
```
###Unfiltered vs Quality Filtered Counts
```{r,echo=TRUE,eval=TRUE,include=TRUE,messages=TRUE,error=TRUE}
count.table
```
