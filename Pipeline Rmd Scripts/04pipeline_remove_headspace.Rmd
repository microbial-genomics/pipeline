#Remove whitespace from file headers
...
sample.id.3 <- NA
for(i in 1:nfiles.2){
  sample.id.3[i] <- substring(filtered.dir.wpath[i],55,57) #hard coded substring of SFBR samples
}
sample.id.3
...
```
##Remove whitespace file command
...
no.whitespace.wpath <- paste(modified.write.dir,list.files(modified.write.dir),sep="")
```
###Removed whitespace files
```{r,echo=TRUE,eval=TRUE,include=TRUE,messages=TRUE,error=TRUE}
no.whitespace.wpath
```